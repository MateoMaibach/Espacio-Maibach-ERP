...
router.get("/:id", (req, res) => {
  ...
  const fichero = db.select().from(ficheros).where(eq(ficheros.clienteId, req.params.id!)).get();
...
router.put("/:id", (req, res) => {
  const existing = db.select().from(clientes).where(eq(clientes.id, req.params.id!)).get();
  if (!existing) {
    res.status(404).json({ error: "Cliente no encontrado" });
    return;
  }
  ...
  }).where(eq(clientes.id, req.params.id!)).run();
...
router.delete("/:id", (req, res) => {
  ...
  db.delete(clientes).where(eq(clientes.id, req.params.id!)).run();
  res.json({ ok: true });
});
